...
if __name__ == "__main__":
    ...
    lb = X_star.min(0)
    ub = X_star.max(0)
    lub_sum = np.array([[[0, 0], [0.33, 0.33]], [[0.33, 0], [0.66, 0.33]],[[0.66, 0], [0.99, 0.33]],
              [[0, 0.33], [0.33, 0.66]], [[0.33, 0.33], [0.66, 0.66]],[[0.66, 0.33], [0.99, 0.66]],
              [[0, 0.66], [0.33, 0.99]], [[0.33, 0.66], [0.66, 0.99]],[[0.66, 0.66], [0.99, 0.99]]])
    ...
    for epoch in range(iteration + 1):
        if epoch % 10000 == 0:
            re = []
            X_f_train = lb + (ub - lb) * np.random.rand(int(N_f * (1 - (epoch / iteration))), 2)
            for i, d in enumerate(lub_sum):
                ...
                xx_c = x_f_train[:, 0:1]
                tt_c = x_f_train[:, 1:]
                xx_c = ms.Tensor(xx_c, dtype=ms.float32)
                tt_c = ms.Tensor(tt_c, dtype=ms.float32)
                r_c = PDE(xx_c, tt_c, net)
                r_c = np.average(abs(r_c.asnumpy()))
                re.append([r_c, i])
            sorted_re = sorted(re, key=lambda re: re[0], reverse=True)
            num = int(N_f * epoch / iteration)
            for i in range(3):
        ...
        zero = ops.Zeros()
        ca_all_zeros = zero((np.shape(X_f_train)[0], 1), mindspore.float32)
        xxx = X_f_train[:, 0:1]
        ttt = X_f_train[:, 1:]
        xxx = ms.Tensor(xxx, dtype=ms.float32)
        ttt = ms.Tensor(ttt, dtype=ms.float32)
