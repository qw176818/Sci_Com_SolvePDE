...
for epoch in range(iteration + 1):
    optimizer.zero_grad()
    x_in = np.random.uniform(low = 4.5, high = 7, size = (700, 1))
    ca_in = autograd.Variable(torch.from_numpy(x_in).float(), requires_grad = True)
    ca_out = PDE(ca_in, net)
    ca_all_zeros = autograd.Variable(torch.from_numpy(np.zeros((700, 1))).float(), requires_grad = False)
    ...
    if epoch % 10000 == 0:
        plt.cla()
        pre_in = torch.linspace(4.5, 7, 9000)
        ...
        plt.pause(0.1)
torch.save(net.state_dict(), "ddpinn_sub_d5")