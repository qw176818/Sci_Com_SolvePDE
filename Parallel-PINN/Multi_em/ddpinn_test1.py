...
for epoch in range(iteration + 1):
    optimizer.zero_grad()
    x_in = np.random.uniform(low = 1.5, high = 5, size = (600, 1))
    ...
    if epoch % 10000 == 0:
        plt.cla()
        pre_in = torch.linspace(1.5, 5, 9000)
        pre_in = torch.reshape(pre_in, (9000, 1))
        y = torch.sin(w1 * pre_in) + torch.sin(w2 * pre_in)
        y_train0 = torch.tanh(w2 * (pre_in - torch.pi)) * net(pre_in)
        ...
        plt.pause(0.1)
torch.save(net.state_dict(), "ddpinn_sub_d4")
