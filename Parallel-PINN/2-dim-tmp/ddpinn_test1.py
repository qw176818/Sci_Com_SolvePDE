...
for epoch in range(iteration + 1):
    ...
    x_in = np.random.uniform(low = -2.5, high = 2.5, size = (1200, 1))
    ca_in = autograd.Variable(torch.from_numpy(x_in).float(), requires_grad = True)
    ca_out = PDE(ca_in, w, net)
    ca_all_zeros = autograd.Variable(torch.from_numpy(np.zeros((1200, 1))).float(), requires_grad = False)
    ...
    if epoch % 10000 == 0:
        plt.cla()
        pre_in = torch.linspace(-2.5, 2.5, 9000)
        ...
        plt.pause(0.1)
torch.save(net.state_dict(), "./ddpinn_sub_d1")